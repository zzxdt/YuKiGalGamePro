      ...
      <v-row>
        <v-col sm="12" md="5" lg="4">
          <v-text-field
            class="text-center"
            variant="underlined"
            :label="$t('TranslatorSettings.currentTranslationApiType')"
            readonly
            dense
            v-model="textTranslationApi"
          ></v-text-field>
        </v-col>
        <v-col sm="12" md="5" lg="4">
          <v-text-field
            class="text-center"
            variant="underlined"
            :label="$t('TranslatorSettings.currentMecabApiType')"
            readonly
            dense
            v-model="mecabTranslationApi"
          ></v-text-field>
        </v-col>
        <v-col sm="12" md="2" lg="4">
          <v-btn color="success" @click="SureSet">{{
            ...
            <v-row>
              ...
              <v-col sm="12" md="4" lg="2">
                <v-select
                  ...
                  v-model="api.apiType"
                  variant="underlined"
                  :disabled="api.selectSwitchDisable"
                  :label="$t('TranslatorSettings.selectApiType')"
                ></v-select>
                <v-switch
                  color="primary"
                  v-model="api.enable"
                  :label="`${api.name}是否打开:${api.enable}`"
        ...
        <v-card>
          ...
          <v-card-text>
            <v-card-subtitle class="d-flex justify-center">{{
              $t('TranslatorSettings.deleteOrSend')
            }}</v-card-subtitle>
          </v-card-text>
          <v-spacer></v-spacer>
...
const setupApiTypeWatchers = () => {
  ...
  onlineApis.value.forEach((api, index) => {
    ...
    const stopApiType = watch(
      ...
      (newVal, oldVal) => {
        if (newVal !== oldVal) {
          const duplicates = onlineApis.value.filter(
            (item, idx) => idx !== index && item.apiType === newVal
          )
