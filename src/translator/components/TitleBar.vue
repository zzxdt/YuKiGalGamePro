  ...
  <div class="titleBar">
    ...
    >
      <div class="buttonManager">
        <v-btn
          @click="pauseTraText"
          :icon="puaseTranslationText ? 'mdi-play' : 'mdi-pause'"
          color="tertiary"
        >
        </v-btn>
        <v-btn
          ...
          color="tertiary"
          :icon="isAlwaysOnTop ? 'mdi-lock' : 'mdi-lock-open-outline'"
        ></v-btn>
        <v-btn
          color="tertiary"
          @click="saveConfig"
          :icon="IsSaveConfig ? 'mdi-content-save-check' : 'mdi-content-save'"
        ></v-btn>
        <v-btn color="tertiary" @click="closeWindows" icon="mdi-close"> </v-btn>
      </div>
    </v-app-bar>
  </div>
</template>
...
<script setup>
import { onMounted, defineProps, computed, onBeforeUnmount, getCurrentInstance } from 'vue'
