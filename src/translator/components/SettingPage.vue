    ...
    <v-container class="settingPage">
      <v-row no-gutters align="center">
        ...
        <v-col>
          ...
          <v-slider step="0.1" :thumb-label="originalTextThumbLabel" min="0.1" max="2"
            v-model="originalTextSize"></v-slider>
        </v-col>
      </v-row>
      <v-row no-gutters align="center">
        <v-col cols="3">
        ...
        <v-col>
          <p style="color: white">{{ $t('TranslatorSettingsPage.size') }}</p>
          <v-slider step="0.1" :thumb-label="translationTextThumbLabel" min="0.1" max="2"
            v-model="translationTextSize"></v-slider>
        </v-col>
        <v-col>
          <p style="color: white">{{ $t('TranslatorSettingsPage.margin') }}</p>
          <v-slider step="0.1" class="margin-top" :thumb-label="translationTextMarginThumbLabel" min="0.1" max="2"
            v-model="translationTextMargin"></v-slider>
        </v-col>
      </v-row>
      <v-row no-gutters align="center">
        <v-col cols="3">
        ...
        <v-col>
          ...
          <v-slider step="0.1" :thumb-label="mecabTextThumbLabel" min="0.1" max="2" v-model="mecabTextSize"></v-slider>
        </v-col>
        <v-col>
          <p style="color: white">{{ $t('TranslatorSettingsPage.mecabFontPadding') }}</p>
          <v-slider step="0.1" :thumb-label="mecabFontPaddingThumbLabel" min="0.1" max="2"
            v-model="mecabFontPadding"></v-slider>
        </v-col>
      </v-row>
      <v-row no-gutters align="center">
        <v-col cols="3">
        ...
        <v-col>
          ...
          <v-slider step="0.1" :thumb-label="mecabReadingThumbLabel" min="0.1" max="2"
            v-model="mecabReadingSize"></v-slider>
        </v-col>
      </v-row>
      ...
      <v-row class="margin-top">
        <v-col class="d-flex justify-center">
          <v-btn color="primary" @click="toggleDevTools">{{ $t('mainSilderBar.toggleDevTools') }}</v-btn>
        </v-col>
        <v-col class="d-flex justify-center">
          <v-btn color="success" @click="settingAchieve">{{ $t('TranslatorSettings.settingAchieve') }}</v-btn>
        </v-col>
      </v-row>
    </v-container>
  </div>
...
const translationTextThumbLabel = computed(() => {
  return useTranslatorConfigStore.translatorWindow.translationText.fontSize !== 0.1 ? 'always' : false;
})
