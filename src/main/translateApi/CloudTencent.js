...
const sign = (secretKey, signStr, date) => {
    let SecretDate = crypto.createHmac('sha256', "TC3" + secretKey).update(date).digest();
...
const formatRequestData = (action, params) => {
    ...
    headers["Authorization"] = `TC3-HMAC-SHA256 Credential=${SecretId}/${CredentialScope}, SignedHeaders=content-type;host, Signature=${signature}`;
